...
export default function APICurlPage() {
  ...
  return (
    <div className="bg-[#f0e0d6] min-h-screen font-mono p-4 flex flex-col items-center space-y-6">
      {/* Header */}
      <h1 className="text-2xl underline font-bold">{data.service}</h1>
      ...
      <form method="post">
        <div className="border border-gray-400 bg-white p-4 w-full max-w-2xl shadow-md space-y-4">
          ...
          <button className="bg-[#d6d6d6] border border-gray-600 px-4 py-1 text-sm font-mono shadow hover:bg-[#c0c0c0]" type="submit">Run Curl</button>
        </div>
      </form>

      <div className="border border-gray-400 bg-white p-4 w-full max-w-2xl shadow-md space-y-4">
        <div>
          <h2 className="underline font-semibold">Curl Response:</h2>
          <div className="bg-gray-100 p-2 text-xs break-all"><pre className="whitespace-pre-wrap bg-gray-100 p-2 border">{actionData?.output}</pre></div>
